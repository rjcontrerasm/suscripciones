...
<button type="button" class="btn btn-sm btn-outline-secondary" title="Ver detalle" onclick="toggleEdit('view-renovacion-<?= $r['id']; ?>')">👁</button>
<button type="button" class="btn btn-sm btn-primary" onclick="toggleEdit('edit-renovacion-<?= $r['id']; ?>')">Editar</button>
<form class="d-inline" method="POST" action="/renovaciones/eliminar" onsubmit="return confirm('¿Eliminar renovación?')"><input type="hidden" name="csrf_token" value="<?= csrf_token(); ?>"><input type="hidden" name="id" value="<?= $r['id']; ?>"><button class="btn btn-sm btn-outline-danger">Eliminar</button></form>
</td></tr>
