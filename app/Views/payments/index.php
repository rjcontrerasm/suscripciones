...
<button type="button" class="btn btn-sm btn-secondary" onclick="toggleEdit('view-pago-<?= $p['id']; ?>')">👁</button>
<button type="button" class="btn btn-sm btn-outline-primary" onclick="toggleEdit('edit-pago-<?= $p['id']; ?>')">Editar</button>
<form class="d-inline" method="POST" action="/pagos/eliminar" onsubmit="return confirm('¿Eliminar pago?')"><input type="hidden" name="csrf_token" value="<?= csrf_token(); ?>"><input type="hidden" name="id" value="<?= $p['id']; ?>"><button class="btn btn-sm btn-danger">Eliminar</button></form>
</td></tr>
